...
- **Toolbar geometry frame-rate fanout.** The deadband short-circuit must run inside the scene's `useFrame` *before* calling `selectionMetaActions.setToolbarGeometry`. Confirm with a perf trace covering idle camera tracking on a selected item (not just drag) — see Performance verification below.
