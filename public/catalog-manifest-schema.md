...
- If the manifest fetch fails, the app falls back to a static built-in catalog
- Manifest paths are resolved relative to the app's `import.meta.env.BASE_URL`
- Failed asset preloads do not block editor startup; the user can manually retry from the startup error overlay
