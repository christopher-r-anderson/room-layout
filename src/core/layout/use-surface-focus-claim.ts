...
export function useSurfaceFocusClaim(surface: FocusableSurface) {
  ...
  // The object identity must be stable, not only its members: consumers put it
  // in ref-callback deps, and an identity change makes React detach/reattach
  // the ref - firing claimRef(null) and wrongly releasing a held claim.
  return useMemo(
