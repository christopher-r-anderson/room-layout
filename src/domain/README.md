...
- `environment-materials.ts` - room finish and lighting vocabulary
  (`FloorFinishOption`, `WallFinishOption`, `LightingMoodOption`,
  `EnvironmentMaterialConfig`) and pure finish/mood lookups.
...
- `geometry/` - pure placement/geometry functions over the model (footprint,
  layout, spawn, wall clearance, room metrics), shared by core, scene, and
  features.
