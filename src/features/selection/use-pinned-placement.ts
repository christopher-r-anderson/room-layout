...
// React wrapper around resolveHeldPlacement, using the "store the previous value
// in state" pattern so it reads no refs during render. The held placement is
// (re)captured from the live one only at the moment the pin engages, so the
// frozen-while-pinned path never calls setState — camera motion under a pinned
// toolbar costs no extra render. `resetKey` force-releases the hold when the
// placement context changes (a new selection or geometry source) so a stale
// pinned position can never bleed onto a different object.
