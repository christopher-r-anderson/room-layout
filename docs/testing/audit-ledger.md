...
| core/operations/focus-actions.test.ts              | keep    | `requestFocus` resolution/origin-fill and the pending-focus reconciler, asserted against `focus-store` state (no collaborator spies).                                                                                                                                                                                                    |
...
| core/stores/feedback-store.test.ts                 | keep    | The routing contract (per entry point: fired surface AND silent surfaces) plus the announcer mechanics (nonce re-announce, movement debounce, reset); browser twin e2e/feedback-routing.spec.ts.                                                                                                                                         |
| app/chrome/announcer.test.tsx                      | keep    | Live-region DOM contract: always-mounted channels, aria attributes, keyed-node re-announce.                                                                                                                                                                                                                                              |
...
| core/stores/selection-store.test.ts                | keep    | Id-only selection pointer lifecycle (set/overwrite/clear/reset).                                                                                                                                                                                                                                                                         |
| core/stores/focus-store.test.ts                    | keep    | Claim-guarded focused-surface tracking + pendingFocus lifecycle (last-write-wins, guarded realization).                                                                                                                                                                                                                                  |
...
| core/operations/catalog-manifest.test.ts                                | keep    | Well-structured; mocks only true boundaries. Expand: floor diffuse/normal invalid-path branch (source `:329-341`).                                                                  |
...
| core/layout/editor-rects-context.test.tsx         | keep    | Verifies split-context wiring + exact guard messages.                                                                                                                                                                                                                                 |
| shared/layout/use-header-layout-mode.test.ts      | keep    | Both branches: matchMedia-absent fallback + live breakpoint tracking.                                                                                                                                                                                                                 |
| shared/layout/use-rect-registry.test.ts           | keep    | Thorough registration/observe/refresh/disconnect coverage.                                                                                                                                                                                                                            |
...
- ↪ DOM-measurement mock setup (`use-element-rect`/`use-element-size`/`use-rect-registry`) — optional; not pursued.
