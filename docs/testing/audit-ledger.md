...
| core/operations/focus-actions.test.ts              | fix     | `requestOutlinerFocus` tests (`:51-54,62-65,73-76`) spy on the collaborator instead of asserting resulting `useSelectionFocusStore` state (as the reconciler tests do at `:108,112,123`).                                                                                                                                                |
...
| core/feedback/announcement-store.test.ts           | keep    | Nonce re-announce, movement debounce, cancel-on-explicit-announce, reset — all behavioral through the store's public surface.                                                                                                                                                                                                            |
| core/feedback/feedback.test.ts                     | keep    | The routing contract: per entry point, fired surface AND silent surfaces. Normative twin of docs/architecture/feedback.md + e2e/feedback-routing.spec.ts.                                                                                                                                                                                |
| app/chrome/feedback/announcer.test.tsx             | keep    | Live-region DOM contract: always-mounted channels, aria attributes, keyed-node re-announce.                                                                                                                                                                                                                                              |
