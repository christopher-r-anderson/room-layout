...
**Feedback-surfaces migration note:** the three `url-restore` "status line
clears on next action" tests were deleted WITH their contract (the visible
status line no longer exists; errors are persistent toasts with explicit
dismissal). Their successor coverage is the toast lifecycle in
`feedback-toasts.spec.ts` — a contract replacement, not a coverage loss.
...
| `startup-loading.spec.ts`                 | genuine-e2e           | Loading screen gates the editor until assets resolve; real network-route interception, event-gated on the loader lifecycle. Clean.                                                                                                                                                                                                                                               |
...
| `editor-a11y-audits.spec.ts`              | genuine-e2e           | axe over shell/dialogs/outliner/selected/empty — needs the real computed a11y tree. Scans are scoped to WCAG 2.2 A/AA tags with zero disables (`axe.ts`). Minor: redundant trailing `itemCount).toBe(0)` (`:78`) after `waitForItemCount(0)` (`:73`).                                                                                                                            |
| `feedback-a11y-audits.spec.ts`            | genuine-e2e           | axe over the feedback states the happy-path chain cannot reach: toasts (incl. over an open drawer), startup loading/error overlays, field errors, the F6-focused viewport. Per-test route setup before goto, so it stays separate from `editor-a11y-audits`.                                                                                                                     |
| `feedback-routing.spec.ts`                | genuine-e2e           | Pins the event-class → surface routing policy in the real DOM (fired surface AND silent surfaces). Normative twin of `docs/architecture/feedback.md` + `core/stores/feedback-store.test.ts`; the three change together.                                                                                                                                                          |
| `feedback-toasts.spec.ts`                 | genuine-e2e           | Toast lifecycle only a browser shows: error persistence vs success auto-dismiss, stacking + limit hiding, F6 focus semantics, shell quiescence via perf counters.                                                                                                                                                                                                                |
