...
This document describes runtime core responsibilities, boundaries, and
data flow between app shell composition, core stores, and the scene
domain.

## Purpose and Scope

The core layer provides shared state seams for cross-feature behavior.
...
It is also the home for **cross-cutting operations**: module-level
...
`src/core/dialog-store.ts` owns only top-level dialog surface state,
...
`src/core/dialog-contract.ts` defines the feature-facing dialog contract
...
defined in `src/core/dialog-store.ts`.
...
or operation-level intent handling.
...
Cross-store writes stay in `core/operations` (or
...
functions in `core`, not as app controller hooks. They read/write the
...
- `scene-reset`, `scene-draft`, `scene-url`, `restore-flow` — scene-persistence
  ...
  orchestration). Homed here so operations can use them without a
  feature-to-feature import.

Feature-internal orchestration (for example `features/selection/deletion-actions`,
`features/catalog/catalog-actions`) stays in the owning feature and imports
core operations rather than sibling features.
...
App/core own orchestration and UI-facing state seams.
...
3. Dialog definitions and core operations read through store selectors
   and context seams.
...
5. Cross-cutting side effects (announcements, focus reconciliation, restore
   messaging) run in `core/operations`.
...
- feature guard logic: dialog definition `canOpen(...)` and/or feature
  operation intent checks
- focus target resolution to DOM: top-header or owning UI
...
- projection-driven geometry data still comes from scene snapshot seams where
  needed
- startup restore orchestration is a core operation (the
