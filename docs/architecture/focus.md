...
Producers state what should be focused rather than where: pane commands name a
...
`focus-policy.test.ts`; that file holds the table.
...
The spatial scene and the item list are distinct focus targets. The resolver
picks between surfaces on exactly three grounds - operation type is never one
of them:
...
- Pointer gestures never move focus - unless the operation destroyed the
  ...
  rather than left on the body, whatever the input device.
...
- **Supersession**: `requestFocus` writes directives and every resolution
  supersedes - it writes a directive or clears the pending one. The one other
  writer is the desktop toolbar site, which forwards an item-actions
...
- **World changes re-validate**: the pending-focus reconciler clears on
  ...
  must clear or re-validate the same way. Known edge: pane shortcuts can write
  a directive during the brief window before the lazy chrome chunk mounts its
  realizer;
