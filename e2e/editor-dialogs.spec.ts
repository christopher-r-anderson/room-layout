...
test('desktop camera presets clear the open Room sidebar with a gap', async ({
  ...
  const roomSurface = page.getByRole('complementary', { name: 'Room' })
  const cameraTools = page.getByRole('toolbar', { name: 'Camera' })
  await expect(roomSurface).toBeVisible()
  await expect(cameraTools).toBeVisible()

  const roomBox = await roomSurface.boundingBox()
  const cameraBox = await cameraTools.boundingBox()

  if (!roomBox || !cameraBox) {
